...
#AutoIt3Wrapper_Res_ProductVersion=0.6.5
...
#Region GUI Creation
	Func CreateGUI()
		...
		$g_hGUI = GUICreate("UT2004 Community Installer v0.6.4", 640, 480, -1, -1, _
...
#Region Installation Process
	...
	Func Phase2_DownloadISO()
		...
		Local $sPortableISO = $g_sPortableDir & "\UT2004.ISO"
	...
	Func Phase_Finalise()
		; WHAT: Move _Downloads cache and clean up temp files
		; WHY: Must run AFTER all bonus packs and patch so all downloads are present
		;      before we move the _Downloads folder to its final location
		; HOW: If Keep Files checked - DirMove _Downloads to @ScriptDir\_Downloads
		;      If Keep Files unchecked - delete temp directory
		;
		; RETURN: True always (non-critical)
		
		LogMessage("Starting Phase Finalise: Keep Files and Cleanup")
		...
		If $bKeepFiles Then
			...
			If FileExists($g_sDownloadDir) Then
				...
				; WHAT: Move entire _Downloads folder (archives only - extracted content is outside)
		...
		Else
			...
			LogMessage("'Keep installer files' is unchecked - cleaning up temp directory")
	...
	Func Phase6_InstallMegaPack()
		...
		; Check for cached files - portable dir first, then temp downloads
		; Portable: @ScriptDir\_Downloads (moved here after previous install with Keep Files)
		; Temp: %TEMP%\UT2004_Install\_Downloads (from current/previous session)
		Local $sCacheDir = $g_sPortableDir
		If Not FileExists($sCacheDir) Then $sCacheDir = $g_sDownloadDir
		Local $bAllCached = True
		Local $aCacheFiles[4]
		$aCacheFiles[0] = $sCacheDir & "\_MegaPack\UT2004MegaPack.z01"
		$aCacheFiles[1] = $sCacheDir & "\_MegaPack\UT2004MegaPack.z02"
		$aCacheFiles[2] = $sCacheDir & "\_MegaPack\UT2004MegaPack.z03"
		$aCacheFiles[3] = $sCacheDir & "\_MegaPack\UT2004MegaPack.zip"
		
		For $i = 0 To 3
			If Not FileExists($aCacheFiles[$i]) Then
				$bAllCached = False
				ExitLoop
			EndIf
		Next
		...
		If $bAllCached Then
			; All parts found in cache - copy to temp and skip download
			UpdateStatus("MegaPack found in cache, skipping download...")
			LogMessage("MegaPack cache found in: " & $sCacheDir)
			For $i = 0 To 3
				FileCopy($aCacheFiles[$i], $aPartFiles[$i], 1)
			Next
			LogMessage("MegaPack parts copied from cache")
		Else
		...
		EndIf ; End cache/download block
	...
	Func Phase7_InstallCBP1()
		...
		Local $bAllCached = True
		Local $aCacheFiles[3]
		$aCacheFiles[0] = $sCacheDir & "\_CBP1\CBP1.z01"
		$aCacheFiles[1] = $sCacheDir & "\_CBP1\CBP1.z02"
		$aCacheFiles[2] = $sCacheDir & "\_CBP1\CBP1.zip"
		...
		If $bAllCached Then
			UpdateStatus("CBP1 found in cache, skipping download...")
			LogMessage("CBP1 cache found in: " & $sCacheDir)
			For $i = 0 To 2
				FileCopy($aCacheFiles[$i], $aPartFiles[$i], 1)
			Next
			LogMessage("CBP1 parts copied from cache")
		Else
	...
	Func Phase8_InstallCBP2V1()
		...
		Local $bAllCached = True
		Local $aCacheFiles[4]
		$aCacheFiles[0] = $sCacheDir & "\_CBP2V1\cbp2_volume1.z01"
		$aCacheFiles[1] = $sCacheDir & "\_CBP2V1\cbp2_volume1.z02"
		$aCacheFiles[2] = $sCacheDir & "\_CBP2V1\cbp2_volume1.z03"
		$aCacheFiles[3] = $sCacheDir & "\_CBP2V1\cbp2_volume1.zip"
		...
		If $bAllCached Then
			UpdateStatus("CBP2 Volume 1 found in cache, skipping download...")
			LogMessage("CBP2V1 cache found in: " & $sCacheDir)
			For $i = 0 To 3
				FileCopy($aCacheFiles[$i], $aPartFiles[$i], 1)
			Next
			LogMessage("CBP2V1 parts copied from cache")
		Else
			; Download all 4 parts
	...
	Func Phase9_InstallCBP2V2()
		...
		Local $bAllCached = True
		Local $aCacheFiles[4]
		$aCacheFiles[0] = $sCacheDir & "\_CBP2V2\cbp2_volume2.z01"
		$aCacheFiles[1] = $sCacheDir & "\_CBP2V2\cbp2_volume2.z02"
		$aCacheFiles[2] = $sCacheDir & "\_CBP2V2\cbp2_volume2.z03"
		$aCacheFiles[3] = $sCacheDir & "\_CBP2V2\cbp2_volume2.zip"
		...
		If $bAllCached Then
			UpdateStatus("CBP2 Volume 2 found in cache, skipping download...")
			LogMessage("CBP2V2 cache found in: " & $sCacheDir)
			For $i = 0 To 3
				FileCopy($aCacheFiles[$i], $aPartFiles[$i], 1)
			Next
			LogMessage("CBP2V2 parts copied from cache")
		Else
			; Download all 4 parts
...
#Region Utility Functions
	...
	Func InitializeLog()
		...
		LogMessage("UT2004 All-In-One Installer v0.6.4")
	...
	Func CreateTab1_Installation()
		...
		$aControls[$iIdx] = GUICtrlCreateLabel("v0.6.4", 20, $iContentY + 35, 600, 15, $SS_CENTER)
		GUICtrlSetFont(-1, 8)
	...
	Func CreateTab3_Options()
		...
		; Resolution label
		$aControls[$iIdx] = GUICtrlCreateLabel("Configure default resolution - Coming Soon", 45, $iContentY + ($iSpacing * 3) + 2, 570, 20)
		GUICtrlSetColor(-1, $COLOR_TEXT_DIM)
		GUICtrlSetBkColor(-1, $GUI_BKCOLOR_TRANSPARENT)
		$iIdx += 1
		...
		; Refresh rate label
		$aControls[$iIdx] = GUICtrlCreateLabel("Configure refresh rate - Coming Soon", 45, $iContentY + ($iSpacing * 4) + 2, 570, 20)
		GUICtrlSetColor(-1, $COLOR_TEXT_DIM)
		GUICtrlSetBkColor(-1, $GUI_BKCOLOR_TRANSPARENT)
		$iIdx += 1
		
		; Pre-configure game settings checkbox (disabled)
		$aControls[$iIdx] = GUICtrlCreateCheckbox("", 20, $iContentY + ($iSpacing * 5), 20, 20)
		GUICtrlSetState(-1, $GUI_UNCHECKED)
		GUICtrlSetState(-1, $GUI_DISABLE)
		$iIdx += 1
		
		; Game settings label
		$aControls[$iIdx] = GUICtrlCreateLabel("Pre-configure game settings (graphics, controls, etc.) - Coming Soon", 45, $iContentY + ($iSpacing * 5) + 2, 570, 20)
		GUICtrlSetColor(-1, $COLOR_TEXT_DIM)
		GUICtrlSetBkColor(-1, $GUI_BKCOLOR_TRANSPARENT)
		$iIdx += 1
